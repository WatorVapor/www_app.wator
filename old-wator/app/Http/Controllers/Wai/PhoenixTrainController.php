...
class PhoenixTrainController extends Controller
{
    public function fetchTask($msgJson) {
        //
        try {
            $apiStr = file_get_contents('/nativeapi/train.phoenix.fetch.api.json');
            //var_dump($apiStr);
    ...
    public function fetch($tag)
    {
        try
        {
            ...
            if($num %1 == 0) {
                ...
                $url = $this->fetchTask(['type'=>'phoenix','lang'=>'cn']);
                if($url){
                    $task['url'] = $url;
                ...
                $url = $this->fetchTask(['type'=>'phoenix','lang'=>'ja']);
                if($url){
                    $task['url'] = $url;
                    return  response()->json($task);
                }
    ...
    public function update(Request $request,$tag)
    {
        try
        {
            ...
            $apiStr = file_get_contents('/nativeapi/train.phoenix.save.api.json');
            //var_dump($apiStr);
            $apiJson = json_decode($apiStr,true);
            //var_dump($apiJson);
            $waiPort = intval($apiJson['port']);;
